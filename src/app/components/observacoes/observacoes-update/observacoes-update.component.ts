...
@Component({
  selector: 'app-observacoes-update',
  templateUrl: './observacoes-update.component.html',
  styleUrls: ['./observacoes-update.component.css']
})
export class ObservacoesUpdateComponent implements OnInit {
  ...
  campoObs = new FormControl('', [Validators.minLength(5), Validators.maxLength(1000)])
  ...
  update(): void {
    this.service.update(this.obs).subscribe((resposta) => {
      this.router.navigate(['observacoes'])
      this.service.message('Observações atualizado com sucesso!')
    })
  }
